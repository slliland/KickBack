...
router.get('/trends', async (req, res) => {
  const { team_codes } = req.query; // Expecting a comma-separated string, e.g., "GER,ESP,ITA"
  ...
  try {
    ...
    result.rows.forEach(row => {
      ...
      teams.forEach((team, index) => {
        if (codesArray.includes(team)) {
          ...
          const redCardCount = row.red_cards ? row.red_cards.filter(e => e.country_code === team).length : 0;
          const penaltyCount = row.penalties ? row.penalties.filter(e => e.country_code === team).length : 0;
    ...
    const response = Object.keys(teamStats).map(team => ({
      team_code: team,
      stats: Object.values(teamStats[team]).map(stat => ({
        ...stat,
        avg_temperature: stat.temp_count ? (stat.temp_sum / stat.temp_count).toFixed(1) : null,
